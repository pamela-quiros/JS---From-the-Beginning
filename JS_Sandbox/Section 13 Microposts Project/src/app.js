...
function submitPost() {
    ...
    //Validate input
    if (title === '' || body === '') {
        ui.showAlert('Please field in all fields', 'alert alert-danger');
    } else {
...
function deletePost(e) {
    ...
    if (e.target.parentElement.classList.contains('delete')) {
        ...
        if (confirm('Are you sure?')) {
            http.delete(`http://localhost:3000/posts/${id}`).then(data => { ui.showAlert('Post removed', 'alert alert-success'); getPosts(); }).catch(err => console.log(err));
        }
    }
}
